...
export function * getCoupon (api, action) {
  const { data } = action
  // make the call to the api
  const response = yield call(api.getcoupon, data)

  // success?
  if (response.ok) {
    // You might need to change the response here - do this with a 'transform',
    // located in ../Transforms/. Otherwise, just pass the data back from the api.
    yield put(CouponActions.couponSuccess(response.data))
  } else {
    if (response.status !== null) {
      if (response.status < 500) {
        const data = { code: response.status, message: response.data.message }
        yield put(CouponActions.couponFailure(data))
      } else {
