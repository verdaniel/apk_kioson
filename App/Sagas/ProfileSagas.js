  ...
  if (response.ok) {
    ...
    if (response.status !== null) {
      if (response.status < 500) {
        ...
        yield put(ProfileActions.getProfileFailure(data))
      } else {
...
export function * updateProfile (api, action) {
  ...
  const response = yield call(api.getProfile, token, data)
  ...
  if (response.ok) {
    ...
    if (response.status !== null) {
      if (response.status < 500) {
        const data = { code: response.status, message: response.data.message }
        yield put(ProfileActions.updateProfileFailure(data))
      } else {
        const data = { code: response.status, message: handleError(response.problem) }
        yield put(ProfileActions.updateProfileFailure(data))
      }
...
export function * getBalance (api, action) {
  ...
  if (response.ok) {
    yield put(ProfileActions.getBalanceSuccess(response.data))
  } else {
    if (response.status !== null) {
      if (response.status < 500) {
        const data = { code: response.status, message: response.data.message }
        yield put(ProfileActions.getBalanceFailure(data))
      } else {
        const data = { code: response.status, message: handleError(response.problem) }
        yield put(ProfileActions.getBalanceFailure(data))
      }
